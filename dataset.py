...
def extract_vertices(lines):
    ...
    vertices = []
    for line in lines:
        # tmp = list(map(int, line.rstrip('\n').lstrip('\ufeff').split(',')[:4]))
        # vertices.append([tmp[0], tmp[1], tmp[2], tmp[1], tmp[2], tmp[3], tmp[0], tmp[3]])
...
def load_annoataion_quard_from_json(p):
    ...
    for item in json_dict['shapes']:
        points = item['points']
        # x_min, y_min, x_max, y_max = map(int, points)  # 原坐标点是顺时针绕一圈
...
class custom_dataset(data.Dataset):
    def __init__(self, img_root_dir, scale=0.25, length=512):
        ...
        self.gt_files = []
        for ext in exts:
            self.img_files.extend(glob.glob(img_root_dir + "*.%s" % ext))
        self.gt_files = [re.sub(r'\.(jpg|JPG|png|PNG)', r'.json', file) for file in self.img_files]
        self.scale = scale
        self.length = length
